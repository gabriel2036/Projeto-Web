...
import { buscarFilmePorNome } from "@/lib/api";


async function run() {
  let test = await buscarFilmePorNome("Amor Vingado");
  console.log(test);
}
...
export default function YouVersePage() {
  ...
  // função que busca filmes e adiciona ao friend
  const handleSelectFriend = async (friend: Friend) => {
    ...
    // aqui você pode customizar a lista de filmes que o usuário desse amigo vai ter
    const nomesDeFilmes = ["Matrix", "Interestelar", "Dune", "Whiplash", "Inception"];

    const filmes = await Promise.all(
      nomesDeFilmes.map(nome => buscarFilmePorNome(nome))
    );

    const updatedFriend: Friend = {
      ...friend,
      movies: filmes.filter(Boolean) as Movie[]
    };

    setSelectedFriend(updatedFriend);
    setFriendProgress((prev) => ({ ...prev, [friend.id]: 0 }));
  };

  const handleSwipe = useCallback(
    (direction: "left" | "right") => {
      if (!selectedFriend || !selectedFriend.movies) return;
      ...
      <div className="bg-[#1F1F26] rounded-[36px] max-w-[1850px] mx-auto p-8 relative shadow-lg">
        ...
        <div className="flex gap-10 items-start">
          ...
          <div className="flex-1 flex items-center justify-center relative min-h-[400px]">
            {selectedFriend && selectedFriend.movies ? (
